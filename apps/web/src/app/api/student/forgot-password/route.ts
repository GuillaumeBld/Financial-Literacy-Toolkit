...
export async function POST(request: NextRequest) {
  try {
    ...
    const { courseCode, email } = body;

    // Validate required fields
    if (!courseCode || !email) {
      return NextResponse.json(
        { error: 'Course code and email are required' },
        { status: 400 }
      );
    }
    ...
    const result = await transaction(async (client) => {
      ...
      const profile = await client.query(
        ...
        [courseData.course_id, email.trim()]
      );
      ...
      const enrollment = await client.query(
        ...
        [profileData.user_id, courseData.course_id]
      );
      ...
      await client.query(
        ...
        [profileData.user_id, courseData.course_id, token, expiresAt.toISOString()]
      );
